...
public class MqttMessageListener { //Mqtt메세지 수신하는 클래스임
    ...
    @Async
    @ServiceActivator(inputChannel = "mqttInputChannel") //mqttInputChannel에서 들어오는 MQTT 메시지를 처리하기 위해 이 메서드를 연결
    public void handleMessage(Message<?> message) { //Message<?>는 수신한 메시지 객체로, 헤더와 페이로드를 포함
        try {
            Thread.sleep(100);
